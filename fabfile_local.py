...
from amilab_instances import (get_tags_for_machine, get_all_instances, tag_instance,
                              get_instance_by_tags, get_crunch_running, get_instance_for_service)
...
@task
def install_amilab_machines(experiment_profile='amilab-crunches.json'):
    # Load up which machines are needed from the experiment profile
    machines = json.load(open(experiment_profile, 'rt'))
...
def generate_hiera_datasources():
    ...
    common_context = {
        'mongo_hostname': get_instance_for_service('mongo').public_dns_name,
        'redis_hostname': get_instance_for_service('redis').public_dns_name,
        'kestrel_hostname': get_instance_for_service('kestrel').public_dns_name
    }
...
def bootstrap_machines():
    ...
    # will be taken from env.hostname_to_manifest, because it's the only sane
    # way I found in fab to do the provisioning in parallel.
    with settings(parallel=False, user='ami'):
        execute('bootstrap_machine', hosts=hostnames)

@task
def bootstrap_machine():
    # http://docs.puppetlabs.com/guides/puppetlabs_package_repositories.html#for-debian-and-ubuntu
    run('cd /tmp; wget http://apt.puppetlabs.com/puppetlabs-release-saucy.deb')
    run('sudo dpkg -i /tmp/puppetlabs-release-saucy.deb')
