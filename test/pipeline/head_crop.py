...
class HeadCropTest(TestCase):
    ...
    @patch.object(HeadCrop, '_crop_head_using_face_detection', return_value = None)
    @patch.object(HeadCrop, '_crop_head_using_skeleton', return_value = None)
    ...
    @patch.object(HeadCrop, 'send_to')
    @patch.object(HeadCrop, '_crop_head_using_face_detection', return_value = None)
    @patch.object(HeadCrop, '_crop_head_using_skeleton', return_value = Image.frombuffer('RGB', (1, 1), base64.b64decode('AAAA')))
    def test_one_image_and_one_skeleton_close_enough(self, skeleton, face_detect, send_to):
        ...
        eq_(face_detect.call_count, 0, "face_detect should only be called if it has "
                                   "a corresponding image and skeleton")
        send_to.assert_called_once_with('face-recognition', {'width': 1, 'image': 'AAAA', 'height': 1})
