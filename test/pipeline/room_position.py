...
@patch.object(RoomPosition, 'add_to_session_store')
class RoomPositionTest(TestCase):
    ...
    def test_not_interesting_message(self, add_to):
        message = { 'sensor_type': 'kinect',
                   'type': 'RGB'}
        rp = RoomPosition()
        rp.process_message(message)
        eq_(add_to.call_count, 0, "add_to should only be called if it has "
                                   " skeleton")
            
    def test_all(self, add_to):
        ''' The sensor is at origin '''
        self.run_test_with(add_to,
                           {'X':0, 'Y':0, 'Z':0, 
    ...
    def run_test_with(self, add_to, sensor_pos, torso_pos, 
        ...
        rp.process_message(message)
        eq_(add_to.call_count, 2) #one for subject_position and one for skeleton_in_room
        add_to.assert_any_call('test:room-position', 1, PositionMatcher(expected_x,
                                                                   expected_y,
                                                                   expected_z,
                                                                   expected))
