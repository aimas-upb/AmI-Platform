...
boolean WiFlyDevice::findInResponse(const char *toMatch,
  ...
  DEBUG_LOG(2, "Entered findInResponse");
  DEBUG_LOG(3, "Want to match:");
  DEBUG_LOG(3, toMatch);
  DEBUG_LOG(4, "Found:");
  ...
  for (unsigned int offset = 0; offset < strlen(toMatch); offset++) {
    ...
    DEBUG_LOG(6, "Offset:");
    DEBUG_LOG(6, offset);
    DEBUG_LOG(4, (char) byteRead);
    DEBUG_LOG(5, byteRead);
...
boolean WiFlyDevice::enterCommandMode(boolean isAfterBoot) {
  ...
  DEBUG_LOG(2, "Entered enterCommandMode");
...
boolean WiFlyDevice::sendCommand(const __FlashStringHelper *command,
  ...
  /*
   */
  DEBUG_LOG(2, "Entered sendCommand");
  DEBUG_LOG(3, "Command:");
  DEBUG_LOG(3, command);
  uart->print(command);
  delay(20);
...
void WiFlyDevice::requireFlowControl() {
  ...
  DEBUG_LOG(2, "Entered requireFlowControl");
