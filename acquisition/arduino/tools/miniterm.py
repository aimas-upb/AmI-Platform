...
# Added logging of received data feature
...
class Miniterm:
    def __init__(self, port, baudrate, parity, rtscts, xonxoff, echo=False, convert_outgoing=CONVERT_CRLF, repr_mode=0, log=None):
        try:
...
3: hex dump everything""",
    ...
    parser.add_option("-l", "--log-file",
        dest = "log_file",
        action = "store",
        help = "Log all received data to file",
        default = None
    )
    ...
    if options.log_file:
        try:
            log = open(options.log_file, 'wb')
        except IOError as (errno, strerror):
            sys.stderr.write("could not open log file %s: %s\n" % (options.log_file, strerror))
            sys.exit(1)
    else:
        log = None
